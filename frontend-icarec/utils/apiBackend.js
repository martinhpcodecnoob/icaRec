...
export const getRecommendedBusinesses = async (userId, accessToken) => {
  try {
    ...
    return {
      status: 500,
      error: "Error del servidor al extraer los negocios recomendados por los usuarios",
    }
  }
}

export const deleteRecommendedBusiness = async (userId, businessId, accessToken) => {
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URI}/api/interaction/deleteRecommendedBusiness/${userId}?businessId=${businessId}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `${accessToken}`
    ...
    console.error("Error del servidor al eliminar el negocio recomendado", error);
    return {
      status: 500,
      error: "Error del servidor al eliminar el negocio recomendado",
    };
  }
};
